...
# Oxforddown
A template for writing a thesis or a book in R Markdown. The manuscript can then be exported to PDF, Microsoft Word, Gitbook (HTML) and ebook formats

# Requirements
- LaTeX for Mac - download and install the MacTeX distribution from [tug.org/mactex/](http://www.tug.org/mactex/) (please be aware that it's quite large - 4.1 gigs!)
- LaTex for Windows- download and install the  MiKTeX distribution from [latex-project.org](https://www.latex-project.org/get/)
...
- The R packages `bookdown`, `Rtools`, `tidyverse`, and `reticulate` (the other packages you need should be automatically installed when you build this project for the first time in RStudio)
